...
[진행] 형상 온도 의존성 재측정 (Mode 1 히스토그램)          <- 지금 여기
       아키텍처 확정의 마지막 전제: RO 갱신 주기(부팅 1회 vs 상시)
...
### (A) ✅ CARRY4 스케일 드리프트 — 정량화 완료

```
탭 지연 온도계수 : +186.9 ppm/°C        (상관 r = −0.93, 신호/산포 ≈ 7σ)
40 → 72 °C      : +5,981 ppm (0.60 %)
5 ns 누적 오차   : 약 30 ps             ← LSB 17 ps의 1.7배
```

**의미:** 상온 교정 LUT를 70 °C에서 그대로 쓰면 5 ns 구간에 30 ps 오차.
**자동 보정의 필요성이 본 칩에서 실측으로 확립됐다** — 논문의 동기(motivation)를
인용이 아니라 자체 데이터로 제시 가능.

Korkan 2022의 캐리체인 관측(*"29 °C까지 지연 감소, 39 °C 이상 증가"*)과 일치.
