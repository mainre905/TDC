...
> **개정** 2026-08-03 — §1.4-A 부호 정정(+187 -> −304~−546 ppm/°C). 원인은 분석
> 코드의 np.unwrap 주기 가정 버그. 상세 `2026-08-03_report.md`.
> **개정** 2026-07-28 — §1.4(온도 실험 실측: CARRY4 +187 ppm/°C 확정, RO 스케일
> 프록시를 자체 측정으로 기각, 형상 드리프트는 잡음에 묻혀 미측정) 추가.
> §2.2 이중기각 표시, §2.4 역할 배분 확정(스케일=DPS 앵커+XADC 트리거 / 형상=RO).
...
[완료] 온도 실험 1차(07-28): CARRY4 +187 ppm/°C 확정 = 보정 필요성 자체 데이터 확보
       RO 스케일 프록시는 부호 반대로 기각. 형상 드리프트는 잡음에 묻혀 미측정
[완료] 형상 온도 의존성(07-29): 0.28 %/°C, 잡음의 15배 -> 형상 불변 아님
       period-4 구조적 재분배(%4==3만 -12%, 나머지 +11%)
[완료] 스케일 부호 판정(08-03): 같은 빌드 두 경로 동시측정 -> 음수 확정
       7/28의 +187 ppm/°C는 np.unwrap 버그였음 (1.4-A 정정)
[진행] 문서 정정 및 아키텍처 확정                            <- 지금 여기
...
## 1.4 온도 실험 실측 (2026-07-28) — 보정 필요량 확정

`python/temp/` 16개 CSV, 40/50/60/70 °C × 3회 = **12 캡처**(동일 비트스트림).
상세는 `2026-07-28_report.md`.

### (A) ✅ CARRY4 스케일 드리프트 — 정량화 완료 (부호 정정됨)

> ⛔ **2026-08-03 정정.** 아래 원래 기재값 `+186.9 ppm/°C(느려짐)`는 **오류**였다.
> 분석 코드의 `np.unwrap` 주기 가정 버그(위상 랩 낙차를 5000 ps로 가정했으나 실제는
> `L × idx_max` = 4875/4946 ps)로 부호가 뒤집혔다. 하드웨어 문제가 아니다.
> 상세: `2026-08-03_report.md` §3.

**확정값 (2026-08-03, 같은 빌드에서 두 경로 동시 측정, 41 → 80 °C):**

```
탭 지연 온도계수 : 음수 — 온도↑ 이면 탭이 빨라진다
    전달함수 경로  : −546 ppm/°C   (τ 17.917 → 17.545 ps)
    히스토그램 경로: −304 ppm/°C   (τ 18.143 → 17.934 ps)
    유효 최대 탭   : 277 → 281      ← 계산 없는 직접 관측
40 → 80 °C 스케일 변화 : 약 −1.2 %
탭 폭 (41 °C)          : 약 18 ps
```

세 지표 모두 같은 방향. 두 경로의 계수 차이(1.8배)는 **서로 다른 탭 구간**을 보기
때문이며, 형상이 온도에 재분배되므로(§1.4-C) 모순이 아니다.

**의미:** 상온 교정 LUT를 80 °C에서 그대로 쓰면 5 ns 구간에 약 60 ps 오차.
**자동 보정의 필요성이 본 칩에서 실측으로 확립됐다** — 논문의 동기(motivation)를
인용이 아니라 자체 데이터로 제시 가능.

> Korkan 2022는 캐리체인에서 *"29 °C까지 지연 감소, 39 °C 이상 증가"* 를 보고했다.
> 본 칩은 41~80 °C 구간에서 **계속 감소**하므로 반전점이 더 높은 것으로 보인다
> (디바이스·공정·전압 차이). 인용 시 이 차이를 명시할 것.

### (B) ⛔ §2.2(RO 스케일 프록시) — **측정으로 기각**

```
CARRY4 지연 : +186.9 ppm/°C   (온도↑ → 느려짐)
RO 지연     :  −37.6 ppm/°C   (온도↑ → 빨라짐)   ← 부호 반대
```

두 구조의 **역온도 의존(temperature inversion) 반전점이 다르기** 때문. 40~72 °C
구간에서 CARRY4는 반전점 위(정상 거동), RO(LUT+라우팅)는 반전점 아래(반전 거동).

→ RO 주파수로 스케일을 추정하면 **보정 부호가 뒤집혀 오차가 2배**가 된다.
→ §2.2는 §1.2에서 이미 선행연구(Pan 2014)로 기각됐으나, 이제 **우리 데이터로도
   부정**된다. Bourdeauducq 2013류 RO 기반 온라인 보정에 대한 **실측 반례**로
   인용 가치가 있는 음성 결과.

> **RO의 본래 역할(랜덤 hit 소스, bin 폭 보정)에는 영향 없음.** hit rate 변화는
> 30 °C에 걸쳐 0.12 %, 무상관성도 유지(07-26 검증).

### (C) ❌ 형상(bin 폭 패턴) 온도 의존성 — **측정 실패**

```
같은 온도 반복 간 INL 잔차 상관 : 평균 0.72   ← 잡음 바닥
다른 온도 간 상관               : 평균 0.65
```

두 값이 겹쳐 **형상 변화를 분해할 수 없었다.** 원인은 방법론:
`capture_trigger`가 위상 스텝당 **hit 1개**만 저장 → 점당 잡음 17~21 ps,
INL 자체 규모 26 ps와 맞먹음.

**→ 다음 실험은 Mode 1 히스토그램으로.** 점당 수십만 hit이 쌓여 잡음이 무시 가능.

### 아키텍처에 남은 미지수

| 성분 | 드리프트 | 측정 수단 | 갱신 주기 |
|---|---|---|---|
| 스케일 | **+187 ppm/°C (확정)** | DPS 앵커 (크리스털 기준) | 온도 변화 감지 시 |
| **형상** | **미측정** | RO code density | **← 아키텍처 확정의 마지막 전제** |

형상이 둔감하면 RO는 "부팅 1회 + 유휴 갱신", 민감하면 "상시 가동"이 된다.
...
> 본문은 남긴다.
>
> 🔴 **판정 갱신 (2026-08-10):** §2.4·§2.5도 **사실상 폐기**됐다. 두 방향에서 무너졌다.
> ① **자체 데이터** — 8/4 CO 탭 전환 후 40→80 °C 무보정 형상 드리프트가 가중 RMS 3.7 ps 로,
>   TDC 양자화 잡음 5.33 ps **아래**다. 단발 측정에서는 재교정이 필요 없어 융합 교정·무정지
>   재교정의 전제가 사라진다. (`2026-08-04_report.md` §3-5, `2026-08-10_report.md` §4-2)
> ② **선행연구** — **Jia et al., IEEE TIM 74, 2025 "Online Dual-Chain Calibration"** 이
>   두 TDL 역할 교대 + RO code density + FSM + 무정지 + 20~80 °C 검증을 모두 발표했다.
>   세션 중 구상한 이중 체인 아키텍처와 점 하나까지 동일하다. (`2026-08-10_report.md` §5-3)
>
> **현행 후보는 §2.6(탭 소스와 온도 안정성) 하나뿐이다.**
...
### ⛔ 2.2 [이중 기각 — Pan 2014(§1.2-A) + 자체 측정(§1.4-B)] RO 기반 스케일 프록시
...
> ⛔ **2026-07-28 실측으로 물리적으로도 불가함이 확인됨.** CARRY4 지연은 +187 ppm/°C
> 로 증가하는데 RO 지연은 −38 ppm/°C로 **감소**한다(반전점 차이). 부호가 반대라
> 이 식으로 보정하면 오차가 2배가 된다. → §1.4-B
>
> **단, 이는 RO의 "스케일 프록시" 용도만 기각한다.** RO의 본래 역할인
> **랜덤 hit 소스(bin 폭 보정용 자극)** 는 유효하며 §2.4의 형상 측정 수단으로 유지된다.
...
= 양자화 한계의 66배(자유발진 확정), 위상 전 구간 커버 → **랜덤 hit 소스로 적합**.

**보정 필요량 실측 (2026-07-28, §1.4):** CARRY4 탭 지연 **+187 ppm/°C**,
40→72 °C에서 5 ns 구간 **30 ps 오차** 누적(LSB의 1.7배). **자동 보정의 동기를
자체 데이터로 확보.** 동시에 RO 지연은 −38 ppm/°C로 **반대 부호** → 스케일 앵커는
**RO가 아니라 DPS(크리스털 기준)** 여야 함이 물리적으로 확정.

**역할 배분 (2026-07-28 확정):**

| 성분 | 측정 수단 | 감시/트리거 | 상태 |
|---|---|---|---|
| 스케일 | **DPS 앵커** (크리스털) | **XADC 온도** (선형, r=−0.93) | ✅ 확정 |
| 형상 | **RO code density** | — | ⏳ 갱신 주기 미정 (§1.4-C) |

> ~~RO 주파수 프록시~~ 는 §1.4-B로 기각. XADC는 "언제 다시 앵커할지"를 판단하는
> **값싼 트리거**로만 쓴다(스케일 자체는 DPS가 직접 측정).
...
### ★ 2.6 [신규 후보 ③] 탭 소스(C vs S)가 온도 안정성을 결정한다 (2026-08-10)

**한 줄:** CARRY4의 어느 출력을 탭으로 쓰느냐 — XOR 출력(S/O) vs 캐리 출력(C/CO) — 가
딜레이라인의 **온도 안정성**을 결정하며, 올바른 선택이면 런타임 재교정이 불필요해진다.

**실측 (빌드 통제 완료, `2026-08-10_report.md`):** 같은 세션에서 O·CO 를 각각 빌드해 비교

| 세트 | 40→80 °C 무보정 가중 RMS | 양자화 잡음(5.33 ps) 대비 |
|---|---|---|
| O 탭 (XOR 출력) | **19.0 ps** | **3.57 ×** — 온도가 오차를 지배 |
| CO 탭 (캐리 출력) | **3.7 ps** | **0.69 ×** — 잡음에 묻힘 |

**정량 차이가 아니라 질적 차이다.** 임계선을 사이에 두고 갈린다.

부수 실측: 정적 선형성도 INL p-p 155.8 → 96.8 ps (−37.8 %, `2026-08-04_report.md` §2-7),
빌드 재현성도 15.1 % → 1.3 %.
...
| | 무엇을 비교했나 | 온도 |
|---|---|---|
| Carra *TNS* 2018 | C/S 16조합 **전수** 비교, CCCC 최적("the classical") | ❌ 상온 정적 DNL 만 |
| Zhang *JPCS* 2020 | — | "look ahead feature" 로 비선형 원인 **언급만** |
| **제안** | C vs S 의 **온도 안정성** | ✅ 40~80 °C, 빌드 통제 |

**실용적 파급:** 현재 학계는 런타임 온라인 교정 하드웨어를 만든다 — Won *TBCAS* 2016
(20,000-deep FIFO + 2×128 카운터 + prefix adder), Pan *TNS* 2014(전용 보조 채널),
Jia *TIM* 2025(이중 TDL + FSM + 외부 클럭). **이 결과는 탭 선택 한 줄로 그 필요의 상당
부분이 사라진다고 말한다.** Jia 2025 가 가장 좋은 대조군이다.

> ⚠️ **지표 혼동 금지.** Jia 의 "rms 20 ps" 는 zero-interval 20만 회의 **단발 정밀도**이고
> 우리 3.7 ps 는 **LUT 형상 드리프트**다. 서로 다른 양이라 직접 비교 불가.

**리스크 (모두 미해결):**

| 항목 | 상태 |
|---|---|
| **재합성 의존성 49 %** | 최대 약점. 소스가 같은데 재빌드만으로 68.6→35.1 ps. **빌드 통계 필수** |
| 칩 1개 · 계열 1종 | Zynq-7000 만. 일반성 주장 불가 |
| 메커니즘 | "O 경로의 XOR 게이트 때문" 은 **가설**. 미검증 |
| **IEEE Xplore 미검색** | **최우선.** ref 에 없던 논문 1편이 올라오자 주력 방향이 죽었다(§2 머리말). 검색 전에는 어떤 방향도 확정 불가 |
