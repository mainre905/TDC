...
> **개정** 2026-07-27 — §1.2(T16 정독: Pan/Won 판정 — 형상불변·스칼라보정 기각),
> §1.3(신규 5편 + TIM 2024 리뷰의 명시적 공백), §2.4/§2.5(신규 차별화 후보:
> Shape–Scale 융합 교정, 자가검증 플랫폼) 추가. §2.1/§2.2는 기각 표시 후 보존.
...
### 0.5 현재 위치 (2026-07-27 갱신)

```
[완료] 파이프라인 · T1 교차검증(자극 편향 실측) · RO 특성화(무상관 확정, 온도 4점)
       · T16 정독(Pan 2014로 형상불변/스칼라보정 기각, §1.2) · 신규 5편 검토(§1.3)
[전환] 구 승부처(형상 불변 R1) 폐기 → 신규 후보 §2.4(융합 교정)·§2.5(자가검증 플랫폼)
[진행] 온도 실험 (회사, Temperature_Experiment_Protocol.md)   <- 지금 여기
       목적 변경: R1 입증(폐기) → CARRY4 vs RO 온도 추적성 = §2.4의 '추적' 단계 검증
[다음] T15 재조사("dual-stimulus calibration", "BIST TDC") → §2.4/§2.5 확정 → 논문 골격 재편
```

> 구버전 항목(R1 승부처 등)은 §1.2 기각 이력 참조.
...
> ⛔ **위 결론은 2026-07-24 T16 정독으로 뒤집혔다. §1.2를 볼 것.**

---

## 1.2 T16 정독 결과 (2026-07-24) — 형상 불변·스칼라 보정 기각

T16a/b/c 원문(사용자 확보)을 정독한 결과, **§2.1(형상 불변)과 §2.2(스칼라 보정)는
선행연구로 확정되어 기각**한다.

### (A) ⛔ Pan et al., IEEE TNS 2014 — 우리 가설 그 자체가 출판돼 있다
...
> the differences are analyzed..."* — 식 (6): 임의 온도 LUT를 상온 LUT에서 유도.
> 검증 결과 대부분 bin에서 ~3 ps 이내 (Fig. 5). Cyclone II.

| §2.1/§2.2에서 하려던 것 | Pan 2014 |
|---|---|
| 형상 불변 가설 정식화 | ✅ 명시적 가정 |
| 온도별 code density로 검증 | ✅ 온도별 LUT 교정·차이 분석 |
| 스칼라 K(T)로 정적 LUT 스케일 | ✅ 식 (6) 구현 |
| 온칩 배율 측정 (외부센서 없이) | ✅ 전용 캐리체인 + tap/tap_d, 클럭주기 눈금자 |
| dead time 없음·자동 | ✅ |

차이는 배율 센서뿐(그들: 전용 캐리체인 / 우리 계획: RO). **원리 동일 → 신규성 없음.**

### (B) Won et al., IEEE TBCAS 2016 — 형상 불변의 '관찰'도 선행

> *"the **bin-width tendency was almost unchanged**... regardless of the temperature"*
> (10→50 °C, 스텝당 51,200샘플). 단 그들은 이를 활용하지 않고 **전체 LUT를 prefix adder로
> 실시간 재계산** (dead time 0). 실시간 교정 11.03 ps-RMS 유지 vs 미적용 43.83 ps-RMS.

추가 판정:
- **"dual-phase" 용어 충돌**: Won의 dual-phase = 병렬 TDL 2개 + duty cycle estimator
  (클럭 스큐 완화). 우리 = 0°/180° coarse 카운터(메타스테빌리티 회피). **메커니즘은 다르나
  이름이 같아** 우리 것은 개명 필요 (예: dual-edge coarse timestamping).
- Won은 960-bin TDL을 6개 클럭영역에 걸쳐 스큐 영향을 정량화 — 우리 "clock region 무영향"
  주장(§2.3)은 320탭이라 애초에 경계를 거의 안 넘는 조건임을 명시해야 함.

### (C) 기타 온도 축 선행

| 문헌 | 내용 | 판정 |
|---|---|---|
| Mao, Sensors 2022 | 다중체인 교차분할로 **구조적** 저온도계수 (0.014 ps/°C, Kintex-7) | 온도 축 경쟁 |
| Alshehry, Sensors 2023 | −75~80 °C 특성화, 민감도 0.0338 ps/°C (집계 스칼라만) | 특성화 선행 |
| Song, Sensors 2020 | 체인 양끝 레지스터로 지연 감시 → LUT 실시간 보상 (60 nm) | 스케일 감시 선행 |

### 판정 요약 (§1.1 표 갱신)

| 항목 | §1.1 판정 | §1.2 정독 후 |
|---|---|---|
| **형상 불변성 (구 R1)** | ✅ 공백 | ⛔ **기각 — Pan 2014가 가정·검증·구현 완료** |
| 스칼라 K(T) 보정 (§2.2) | 후보 | ⛔ **기각 — Pan 2014** |
| Dual-phase coarse | 보류 | ⚠️ 메커니즘은 다름. **개명 후 부기여로만** |
| 온도 실험 T6/T7 | ★승부처 | **재현 실험으로 격하** (수행 가치는 있으나 novelty 아님) |

---

## 1.3 신규 논문 5편 (2026-07-27 추가) — 리뷰가 명시한 공백

### 목록

| 문헌 | 핵심 | 관계 |
|---|---|---|
| **Xia et al., 리뷰 (IEEE TIM 73, 2024)** | FPGA TDC 전체 조망 + **공백 명시** | ★ 방향 결정 |
| Cui & Li, 양방향 Vernier (TIM 69(8), 2020) | RO 2개 주기차 분해능, 양방향 2배 개선. 셀 재사용→선형성 좋음 | 20 °C 고정 측정, 온라인 교정 없음 |
| Xu et al., 고처리량 Vernier (Appl. Sci. 12:7674, 2022) | Vernier 변환시간을 bi-time 보간으로 단축 | 참고 |
| **Carra et al., SoC 자동교정 (IEEE TRPMS 3(5), 2019)** | code density **처리**를 SoC ARM으로 자동화 | 처리 자동화 선점. **자극은 외부** 랜덤 펄스 |
| Chen, Triple-TDC TMR (Electronics 2020) | 기존 보유본 재업로드 | 기존 분석 유지 |

### ★ Xia 리뷰(TIM 2024)가 명시한 미해결 지점 (원문 인용)

1. *"these methods **still rely on manual calibration methods**, such as density testing..."*
   → 교정이 여전히 수동 density test 의존.
2. *"currently reported research results on TDCs **lack a comprehensive summary and
   comparative analysis of the calibration mechanisms**... difficult to evaluate them
   uniformly. However, **in future research, it will be crucial**..."*
   → **교정 메커니즘의 통일된 평가·비교가 부재** — 명시적 future work.
3. *"**evaluating the measurement uncertainty is necessary** to ensure the reliability..."*
   → 측정 불확도 평가 필요. 실험실→실응용 신뢰성으로의 전환 추세.
4. 자원 효율 추세: folded delay line(자원 −50 %, <10 ps), cyclic 구조.

**공백 2·3이 §2.4/§2.5 신규 후보의 직접 근거다.**
...
> ⚠️ **판정 갱신 (2026-07-27):** §2.1·§2.2는 Pan 2014로 **기각**(§1.2). 이력 보존을 위해
> 본문은 남긴다. **현행 후보는 §2.4(융합 교정)·§2.5(자가검증 플랫폼).**

### ⛔ 2.1 [기각 — Pan 2014, §1.2-A] 형상 불변성(shape invariance)의 실험적 미검증
...
### ⛔ 2.2 [기각 — Pan 2014, §1.2-A] MMCM을 온도 안정 기준자로 사용한 RO 2차 보정
...
- **clock region 경계는 무의미**하다는 반직관적 반증
  - ⚠️ 표현 주의(§1.2-B): Won 2016은 960-bin을 6영역에 걸쳐 스큐 영향을 정량화했다.
    우리 320탭은 경계를 거의 안 넘는 조건이므로 "반증"이 아니라 **"단일 영역 내 배치 시
    무영향 확인"** 으로 좁혀 서술할 것.

### ★ 2.4 [신규 후보 ①] Shape–Scale 분리 융합 교정 (2026-07-27)

**한 줄:** RO code density에서 **형상**(상대 bin 폭)을, DPS 전달함수에서 **절대 스케일**
(크리스털 기준 기울기)을 취해 **융합**하는 새 교정 구성법. 수명주기 전체가 온칩.

$$\text{LUT} = \underbrace{\text{shape}_{RO}}_{\text{백그라운드 연속 누적}} \times \underbrace{\text{scale}_{DPS}}_{\text{2.8초 스윕 재앵커}}$$

**실측 근거 — T1 교차검증 (2026-07-23, 동일 빌드 4캡처):**

| 조합 (CAL→VAL) | AFTER DNL/INL [LSB, P-P] | 해석 |
|---|---|---|
| RO→RO | **0.06 / 0.11** | RO는 형상 측정에 탁월 |
| DPS→DPS | 0.42 / 0.35 | DPS 이산스텝(17.857ps≈LSB)이 density에 부적합 |
| DPS→RO / RO→DPS | 2.91/3.16, 1.94/3.72 | **자극 편향 실재 — 단일 자극 교정의 한계 실증** |

→ *"왜 자극을 두 개 쓰나?"* 에 대한 답이 **우리 실측**이다: 하나만 쓰면 편향된다.
  (Tontini 2018의 "두 방법 일치"는 계측기 1 ps 스텝 조건 — 스텝≈LSB이면 깨짐을 우리가 보임)

**보조 실측 (2026-07-26):** RO 무상관성 직접 확인 — f_RO 63.45 MHz, 변동 120 ppm
= 양자화 한계의 66배(자유발진 확정), 위상 전 구간 커버. RO 온도 4점(38.5~67.2 °C):
+45 ppm/°C 평균, **60~65 °C에서 부호 반전**(temperature inversion) → RO 단독 온도계 부적합,
스케일 앵커는 DPS(크리스털)가 맡아야 하는 이유가 됨.

**선행 대비:**

| | 형상 | 스케일 | 비고 |
|---|---|---|---|
| Pan 2014 | 1회 density(불변 가정) | 클럭주기 눈금자(tap/tap_d — 체인>주기 필요) | 분해는 유사 |
| Won 2016 | 연속 랜덤 hit 전체 재계산 | 동일 | 통계 대량 필요 |
| Korkan 2022 | DPS 전달함수 단독 | 동일 | PC/Python 개입 |
| **제안** | **RO density 연속** | **DPS 280점 피팅** (체인 길이 제약 없음) | **분리 필요성을 실측으로 정당화** |

**리스크:** Pan의 shape×scale 분해와 개념적 친족 — 차별점은 (a) 분리의 실험적 정당화,
(b) 완전 온칩 수명주기(구축→검증→추적→재앵커), (c) 체인 길이 제약 없음. T15에서
"dual-stimulus calibration", "stimulus fusion TDC" 키워드 재확인 필수.

### ★ 2.5 [신규 후보 ②] 자가 검증(Self-Validating) TDC 플랫폼 (2026-07-27)

**한 줄:** 교정 **생성**뿐 아니라 교정 **검증·불확도 평가**까지 외부 계측기 0대로 수행하는
구조. Xia 리뷰(§1.3)의 명시적 공백 2·3에 직접 대응.

**기존 연구의 검증 장비 의존:**

| 논문 | 검증에 쓴 외부 장비 |
|---|---|
| Tontini 2018 | SPAD + Keysight 81150A (1 ps 스윕) |
| Won 2016 | CAEN N108A 지연 유닛 |
| Korkan 2022/2026 | AFG3101C + Keysight 53230A + PC |
| Carra 2019 | 외부 랜덤 펄스 (처리만 온칩 ARM) |

**본 설계가 이미 내장한 것:** 딜레이라인 1 + 온칩 자극 2종(DPS 결정론 / RO 통계·무상관
실증) + 온칩 참 시간 기준(loop_cnt×17.857 ps) + 온칩 히스토그램 엔진(BRAM) + 모니터
(RO 카운터·XADC) → cal/val 분리, 자극 간 교차검증, DNL/INL/불확도 **자가 보고**가 전부
칩 안에서 완결.

**포지셔닝:** ②는 ①의 실현·검증 수단. 논문 골격 = *"① 융합 교정을 기여로, ② 플랫폼
위에서 자가 검증으로 입증"*. 온도 실험(T6/T7)이 성공하면 '추적' 단계까지 닫힘.

**리스크:** "플랫폼" 단독으로는 "성능은?" 반문에 취약 → 반드시 ①의 정량 결과(DNL 0.06,
교차 매트릭스, K(T) 추적)로 뒷받침. "BIST TDC", "self-testing TDC" 선행 재조사 필요(T15).
